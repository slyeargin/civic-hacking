...
function rearrangePoints() {
  ...
  var tempWaypoint = waypoints[$traceurRuntime.toProperty(spliceIndex - 1)];
...
function removeWayPoint() {
  ...
  savMarkers[$traceurRuntime.toProperty(i + 1)].setMap(null);
...
function trip() {
  ...
  tmppoints.pop();
  var mode = $('#mode').val();
  var travelMode = google.maps.TravelMode[$traceurRuntime.toProperty(mode)];
  var request = {
    origin: origin,
...
function clearTmpMarkers() {
  ...
  for (var i = 0; i < tmpMarkers.length; i++) {
    tmpMarkers[$traceurRuntime.toProperty(i)].setMap(null);
  }
  tmpMarkers = [];
  $('#info').empty();
  clearDirections();
...
function callOpenDataForResults(activity, radius) {
  ...
  var name;
  var icon;
  var socrataRadius = radius * 1609.34;
  switch (activity) {
    case 'parks':
      key = '74d7-b74t?';
      name = 'park_name';
...
function addActivitiesToMap(activities, name, icon) {
  ...
  $.each(activities, function(i, entry) {
    if (entry.mapped_location) {
      addMarker(entry, entry.mapped_location.latitude, entry.mapped_location.longitude, entry[$traceurRuntime.toProperty(name)], icon, 'temp');
    }
  });
}
...
//# sourceMappingURL=/Users/slyeargin/Documents/Code/javascript/itnerary/app/static/js/es6/compiled/maps.map
